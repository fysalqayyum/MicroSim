...
void populate_matrix(double **Mat, char *tmpstr, long NUMPHASES) {
  ...
  for (i = 0; i < NUMPHASES*(NUMPHASES-1)/2; i++) {
...
void print_double_array(const char * name, const double * arr, const long num, long step)
{
  fprintf(stdout, "\nstep[%ld] %*s = { ", step, max_length, name);
  for (long i = 0; i < num; i++)
  {
    fprintf(stdout, "%le, ", arr[i]);
  }
  fprintf(stdout, " };");
}

void populate_double_array(double *value, char *tmpstr, long size) {
  char **tmp;
  char *str1, *str2, *token;
  char *saveptr1, *saveptr2;
  int i;
...
//   DIM = atoi(tmp[0]);
  ...
  if (strcmp(tmp[0], "F") == 0) {
    assign_buffer_points_conditions(4, BOUNDARY_LEFT, BOUNDARY_RIGHT, BOUNDARY_FRONT, BOUNDARY_BACK, BOUNDARY_TOP, BOUNDARY_BOTTOM);
  }
  for (i = 0; i < len; ++i) {
    free(tmp[i]);
  }
...
//       boundary[0][j].value[2] = VALUE_LEFT;
    ...
    if(boundary[0][j].type == 3) {
      boundary[0][j].proxy[0] = workers_mpi.rows_x-5;
      boundary[0][j].proxy[1] = workers_mpi.rows_x-6;
      boundary[0][j].proxy[2] = workers_mpi.rows_x-7;
    }
    //Boundary X-
    
    //Boundary X+
    if(boundary[1][j].type == 1) {
      boundary[1][j].proxy[0] = workers_mpi.rows_x-5;
      boundary[1][j].proxy[1] = workers_mpi.rows_x-6;
      boundary[1][j].proxy[2] = workers_mpi.rows_x-7;
    }
    if(boundary[1][j].type == 2) {
//       boundary[1][j].value[0] = VALUE_RIGHT;
//       boundary[1][j].value[1] = VALUE_RIGHT;
//       boundary[1][j].value[2] = VALUE_RIGHT;
    ...
    //Boundary Y+
    if(boundary[2][j].type == 1) {
      boundary[2][j].proxy[0] = workers_mpi.rows_y-5;
      boundary[2][j].proxy[1] = workers_mpi.rows_y-6;
      boundary[2][j].proxy[2] = workers_mpi.rows_y-7;
    }
    if(boundary[2][j].type == 2) {
//       boundary[2][j].value[0] = VALUE_FRONT;
//       boundary[2][j].value[1] = VALUE_FRONT;
...
//       boundary[3][j].value[2] = VALUE_BACK;
    ...
    if(boundary[3][j].type == 3) {
      boundary[3][j].proxy[0] = workers_mpi.rows_y-5;
      boundary[3][j].proxy[1] = workers_mpi.rows_y-6;
      boundary[3][j].proxy[2] = workers_mpi.rows_y-7;
    }
    //Boundary Y-
    if (DIMENSION != 2) {
      //Boundary Z+
      if(boundary[4][j].type == 1) {
        boundary[4][j].proxy[0] = workers_mpi.rows_z-5;
        boundary[4][j].proxy[1] = workers_mpi.rows_z-6;
        boundary[4][j].proxy[2] = workers_mpi.rows_z-7;
      }
      if(boundary[4][j].type == 2) {
//         boundary[4][j].value[0] = VALUE_TOP;
//         boundary[4][j].value[1] = VALUE_TOP;
...
//         boundary[5][j].value[2] = VALUE_BOTTOM;
      ...
      if(boundary[5][j].type== 3) {
        boundary[5][j].proxy[0] = workers_mpi.rows_z-5;
        boundary[5][j].proxy[1] = workers_mpi.rows_z-6;
        boundary[5][j].proxy[2] = workers_mpi.rows_z-7;
      }
      //Boundary Z-
    }
    if(j==3) {
    ...
    //X+
    boundary[1][j].points[0] = workers_mpi.rows_x-3;
    boundary[1][j].points[1] = workers_mpi.rows_x-2;
    boundary[1][j].points[2] = workers_mpi.rows_x-1;
    //X+
    
    //Y+
    boundary[2][j].points[0] = workers_mpi.rows_y-3;
    boundary[2][j].points[1] = workers_mpi.rows_y-2;
    boundary[2][j].points[2] = workers_mpi.rows_y-1;
    //Y+
    ...
    if(DIMENSION != 2) {
      //Z+
      boundary[4][j].points[0] = workers_mpi.rows_z-3;
      boundary[4][j].points[1] = workers_mpi.rows_z-2;
      boundary[4][j].points[2] = workers_mpi.rows_z-1;
      //Z+
...
{
  ptr->gradphi      = MallocM(3,NUMPHASES);
  ptr->gradphi_c    = MallocM(3,NUMPHASES);
  ptr->phistagg     = MallocM(3,NUMPHASES);
  ptr->gradchempot  = MallocM(3,NUMCOMPONENTS-1);
...
//     ptr->deltaphi    = tmp + (NUMPHASES) + 2*(NUMCOMPONENTS-1);
//   if (DIMENSION == 3) {
//     tmp = (double *)malloc(54)*sizeof(double));
//     ptr->fdis        = tmp;
//     ptr->fdis_eq     = tmp + 27;
//     ptr->compi       = tmp + NUMPHASES;
//     ptr->composition = tmp + NUMPHASES   + (NUMCOMPONENTS-1);
...
void free_memory_gradlayer(struct gradlayer *ptr) {
  FreeM(ptr->gradphi, 3);
  FreeM(ptr->gradphi_c, 3);
  FreeM(ptr->phistagg, 3);
  FreeM(ptr->gradchempot, 3);
...
void free_memory_lbm_fields(struct lbm_fields *ptr) {
  if ((ptr->fdis)!=NULL) {
    free(ptr->fdis);
  }
}
