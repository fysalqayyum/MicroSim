#ifndef _MS_READ_INP_
#define _MS_READ_INP_
    ...
    if(tmpstr1[0] != '#') {
      ...
      if (strcmp(tmpstr1,"DIMENSION")==0) {
        ...
        if (((NUMCOMPONENTS-1)>0) && (NUMPHASES>0)) {
          ...
          for (i=0; i < 6; i++) {
            boundary[i] = (struct bc_scalars*)malloc(5*sizeof(*boundary[i])); //4=Number of scalar fields
          }
        }
      }
      ...
      else if (strcmp(tmpstr1,"INVERT_GSL")==0) {
        USE_GSL_MATINV = atol(tmpstr2) == 1;
      }
...
//           fprintf(stdout, "ERROR: H5PCC compiler has issues with BINARY file writing. Please use the hdf5 file format using the WRITEHDF5 key. Will revert to ASCII mode.\n");
      ...
      else if ((strcmp(tmpstr1, "Gamma_abc") == 0) && (NUMPHASES > 2)) {
...
//         }
      ...
      else if ((strcmp(tmpstr1, "CONSTRAINED") == 0)) {
        CONSTRAINED = atoi(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "lambda") == 0) && CONSTRAINED){
        lambda = atof(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "LBM") == 0)){
        LBM = atoi(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "LBM_SAVE_FREQ") == 0)){
        lbmSaveFreq = atoi(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "LBM_RESTART") == 0)){
        LBM_RESTART = atoi(tmpstr2) == 1;
      }
      else if ((strcmp(tmpstr1, "NU_LBM") == 0) && LBM){
        nu_lbm = atof(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "rho_LBM") == 0) && LBM) {
        rho_LBM = (double *)malloc(NUMPHASES*sizeof(double));
        populate_double_array(rho_LBM, tmpstr2, NUMPHASES);
      }
      else if ((strcmp(tmpstr1, "nu") == 0) && LBM) {
        nu = atof(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "W0") == 0) && LBM) {
        W_0 = atof(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "gy") == 0) && LBM) {
        g_y = atof(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "dt") == 0) && LBM) {
        dt = atof(tmpstr2);
      }
      else if ((strcmp(tmpstr1, "beta_c") == 0) && LBM) {
        beta_c = (double *)malloc((NUMCOMPONENTS-1)*sizeof(double));
        populate_double_array(beta_c, tmpstr2, NUMCOMPONENTS-1);
      }

      else if ((strcmp(tmpstr1, "eidt_mode") == 0))
      {
        EIDT.mode           = atol(tmpstr2);
        EIDT_SWITCH         = EIDT.mode == 2 || EIDT.mode == 1;
        if (EIDT_SWITCH) {
          EIDT.comp_far_field = (double*)malloc((NUMCOMPONENTS-1)*sizeof(double));
          EIDT.comp_rate      = (double*)malloc((NUMCOMPONENTS-1)*sizeof(double));
          EIDT.comp_center    = (double*)malloc((NUMCOMPONENTS-1)*sizeof(double));
      ...
      else if ((strcmp(tmpstr1, "comp_ff") == 0))
      {
        if (EIDT_SWITCH) {
      ...
      else if ((strcmp(tmpstr1, "rad_coeff") == 0))
      {
        if (EIDT_SWITCH) {
          EIDT.radius_coeff = atof(tmpstr2);
        }
      }
      else if ((strcmp(tmpstr1, "step_coeff") == 0))
      {
        if (EIDT_SWITCH) {
          EIDT.step_coeff = atof(tmpstr2);
        }
      }
      else if ((strcmp(tmpstr1, "step_zero") == 0))
      {
        if (EIDT_SWITCH) {
